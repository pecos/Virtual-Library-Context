 ...
 * 
 * Modified to make it a shared lib
 *
 * Copyright (C) 2016-2024 Igor Krivenko <igor.s.krivenko@gmail.com>
 * Copyright (C) 2024 yinengy (yinengy@utexas.edu)
 *
 ...
 * @param N_ev The number of low-lying eigenvalues we want to compute
*/
int eign(int N, int N_ev) {
  ...
  auto matrix_op = [N](solver_t::vector_const_view_t in,
  ...
  for(int i = 0; i < N_ev; ++i) { // For each eigenpair ...
    ...
    std::cout << i << ": deviation = " << (rhs - lhs).norm() / N << std::endl;
  }
