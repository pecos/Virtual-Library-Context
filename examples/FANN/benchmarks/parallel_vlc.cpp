...
int cascade_train(int vlc_id, const char *cpu_str)
{
	VLC::Context vlc(vlc_id, gettid());
    vlc.avaliable_cpu(cpu_str);
    VLC::register_vlc(&vlc);
    VLC::Loader loader("/usr/local/lib/libfann.so.2", vlc_id, true);
...
int steepness_train(int vlc_id, const char *cpu_str)
{
	VLC::Context vlc(vlc_id, gettid());
    vlc.avaliable_cpu(cpu_str);
    VLC::register_vlc(&vlc);
...
int xor_train(int vlc_id, const char *cpu_str)
{
	VLC::Context vlc(vlc_id, gettid());
    vlc.avaliable_cpu(cpu_str);
    VLC::register_vlc(&vlc);
...
int main() {
    ...
    t[0] = std::thread(steepness_train, 1, "0-11");
    t[1] = std::thread(xor_train, 2, "12-23");
