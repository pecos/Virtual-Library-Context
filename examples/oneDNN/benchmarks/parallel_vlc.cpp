...
void cnn_inference(int vlc_id, const char* cpu_str, dnnl_engine_kind_t engine_kind) {
...
void cnn_training(int vlc_id, const char* cpu_str) {
    VLC::Context vlc(vlc_id, gettid());
    vlc.avaliable_cpu(cpu_str);
    VLC::register_vlc(&vlc);
    VLC::Loader loader("/usr/local/lib/libdnnl.so.3", vlc_id, true);
...
//     t[0] = std::thread(cnn_inference, 1, "0-23", dnnl_cpu);
    t[1] = std::thread(cnn_training, 2, "0-23");
