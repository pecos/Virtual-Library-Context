...
void init_bfs(int num_cores) {
  bfs_G = new galois::SharedMemSys();
  galois::setActiveThreads(num_cores);
  std::cout << "initialize bfs with " << galois::getActiveThreads() << " cores avaliable" << std::endl;
}
